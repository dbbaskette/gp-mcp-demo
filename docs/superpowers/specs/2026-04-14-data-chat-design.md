# data-chat — Custom MCP Chat Client

**Status:** design
**Date:** 2026-04-14
**Repo:** new service inside `gp-mcp-demo` (monorepo-style), directory `data-chat/`
...
```
Browser (React SPA) ── https ──► Caddy ──► data-chat (Spring Boot)
                                              │
...
Caddy gets a new `/chat*` route pointing at `data-chat:8080`. Keys provided in a `.env` file (git-ignored) next to `docker-compose.yml`.
...
│   ├── config/
│   │   └── personas.yaml
│   ├── src/main/java/com/baskettecase/datachat/
│   │   ├── auth/
│   │   ├── chat/
│   │   ├── llm/
│   │   ├── mcp/
...
│       ├── src/
│       └── index.html
├── docker-compose.yml                          # add data-chat service
├── Caddyfile                                   # add /chat route
└── docs/superpowers/specs/
    └── 2026-04-14-data-chat-design.md            # this file
```
